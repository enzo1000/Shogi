...
class Board:
    ...
    #Attribut a chaques cases du plateau les pieces adequates a un debut de partie
    def createBoard(self):
        for row in range(self.row): #Pour chacune des lignes de notre plateau
            self.Board.append([0 for i in range (self.col)])    #On vient creer le nombre de cases pour la ligne correspondante

            for col in range (self.col):    #Celon la position (indice) on ajoute une piece en particulier
    ...
    #Place les pieces dans le tableau Side
    def createSide(self):
    ...
    #Retourne la piece sur la case aux coordonnees donnees
    #Retourne 0 si la case est "vide" (ne contient pas de piece)
    ...
    #Deplace une piece a partir de sa position actuelle et sa nouvelle position en parametre
    def move(self, piece, row, col):
        #Ecriture python de changement de 2 valeurs sur une meme ligne sans passer par une 3eme variable tampon
        self.Board[piece.row][piece.col], self.Board[row][col] = self.Board[row][col], self.Board[piece.row][piece.col]
        piece.pieceMove(row, col)   #On vient actualiser le placement de la piece (row, col et x, y)

    #Construit le terrain avec un fond marron et des cases blanches
    def drawBoard(self):
        self.window.fill(brown)                     #Toute la fenetre en marron
        for row in range(self.row):                 #Pour toutes les lignes
            for col in range(row%2, self.col, 2):   #Pour, si la colonne est pair ou impaire (couleur de la premiere case sur la ligne), la derniere col, une marche de 1 case sur 2
                pygame.draw.rect(self.window, white, (square*(row), square*(col), square, square))

    #Place une piece a une coordonee donne
    def drawPiece(self, piece, Win):
        Win.blit(piece.image, (piece.x, piece.y))

    #Actualise les pieces cote side
    def drawSide(self):
        for row in range(self.row):
            for col in range(self.col + 2):
    ...
    #Avec l utilisation de drawPiece permet d actualiser toutes les pieces sur le terrain
    def drawPieces(self):
        for row in range(self.row):
